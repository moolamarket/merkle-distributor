...
contract UpdatedAccumulatingMerkleDistributor is Ownable {
    ...
    function claim(uint256 _index, address _account, uint256 _amount, bytes32 _merkleRoot, bytes32[] calldata _merkleProof) external {
        require(allowedMerkleRoots[_merkleRoot], 'UpdatedAccumulatingMerkleDistributor: Invalid merkleRoot.');

        uint claimed = claimedAmount[_account];
        require(claimed < _amount, 'UpdatedAccumulatingMerkleDistributor: Drop already claimed.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(_index, _account, _amount));
        require(MerkleProof.verify(_merkleProof, _merkleRoot, node), 'UpdatedAccumulatingMerkleDistributor: Invalid proof.');

        // Mark it claimed and send the token.
        claimedAmount[_account] = _amount;
        require(IERC20(token).transfer(_account, _amount - claimed), 'UpdatedAccumulatingMerkleDistributor: Transfer failed.');
    ...
    function updateRoot(bytes32 _newRoot) public onlyOwner() {
        require(!allowedMerkleRoots[_newRoot], 'UpdatedAccumulatingMerkleDistributor: merkleRoot already exists.');
    ...
    function recover(address _to, bytes calldata _data) external onlyOwner() returns(bool, bytes memory) {
        if (_to == token) {
            require(block.timestamp > timeout, 'UpdatedAccumulatingMerkleDistributor: not timed out yet.');
        }
